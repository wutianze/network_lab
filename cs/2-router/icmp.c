...
{
	fprintf(stderr,"icmp_send_packet,type:%d,code:%d\n",type,code);
	char* packetSend;
	...
	if(type ==0 && code == 0){
		...
		memcpy(eHS->ether_dhost,eH->ether_shost,ETH_ALEN);
                //u32 tmpS = 0;
		ip_init_hdr(ipHS,ntohl(ipH->daddr),ntohl(ipH->saddr),sendL - ETHER_HDR_SIZE, IPPROTO_ICMP);
		//the source ip will be initialized in ip_send_packet
		icmpHS->type = type;
                icmpHS->code = code;               
		
		fprintf(stderr,"type:%d,code:%d\n",icmpHS->type,icmpHS->code);
